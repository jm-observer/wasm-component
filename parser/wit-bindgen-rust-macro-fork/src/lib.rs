...
use wit_bindgen_core::wit_parser::{
    PackageId, Resolve, UnresolvedPackage, WorldId
};
use wit_bindgen_rust::{Opts, Ownership};

// #[proc_macro]
// pub fn generate(input: proc_macro::TokenStream) ->
// proc_macro::TokenStream {     let config =
// syn::parse_macro_input!(input as Config);     config
//         .expand()
        ...
        if input.peek(token::Brace) {
            ...
            for field in fields.into_pairs() {
                match field.into_value() {
                    ...
                    Opt::Skip(list) => opts
                        .skip
                        .extend(list.iter().map(|i| i.value())),
                    Opt::RuntimePath(path) => {
                        opts.runtime_path = Some(path.value())
                    },
                    Opt::BitflagsPath(path) => {
                        opts.bitflags_path = Some(path.value())
                    },
                    Opt::Exports(exports) => opts.exports.extend(
                        exports.into_iter().map(|export| {
                            (
                                export.key.into(),
                                serialize(export.value)
                            )
                        })
                    ),
                    Opt::Stubs => {
                    ...
                    Opt::ExportPrefix(prefix) => {
                        opts.export_prefix = Some(prefix.value())
                    },
                    Opt::AdditionalDerives(paths) => {
                        opts.additional_derive_attributes = paths
                            .into_iter()
                            .map(|p| {
                                p.into_token_stream().to_string()
                            })
                            .collect()
                    },
...
impl Config {
    pub fn expand(self) -> Result<TokenStream> {
        ...
        // Include a dummy `include_str!` for any files we read so
        // rustc knows that we depend on the contents of those
        // files.
...
enum Opt {
    ...
    // Parse as paths so we can take the concrete types/macro names
...
impl Parse for Opt {
    fn parse(input: ParseStream<'_>) -> Result<Self> {
        ...
        if l.peek(kw::path) {
            ...
            let list = Punctuated::<_, Token![,]>::parse_terminated(
                &contents
            )?;
