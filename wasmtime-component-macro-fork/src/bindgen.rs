...
use wasmtime_wit_bindgen::{
    AsyncConfig, Opts, Ownership, TrappableError
};
...
pub fn expand(input: &Config) -> Result<TokenStream> {
    if !cfg!(feature = "async") && input.opts.async_.maybe_async() {
        return Err(Error::new(
            Span::call_site(),
            "cannot enable async bindings unless `async` crate \
             feature is active"
        ));
    }
    ...
    // Include a dummy `include_str!` for any files we read so rustc
    // knows that we depend on the contents of those files.
...
impl Parse for Config {
    fn parse(input: ParseStream<'_>) -> Result<Self> {
        ...
        if input.peek(token::Brace) {
            ...
            let fields =
                Punctuated::<Opt, Token![,]>::parse_terminated(
                    &content
                )?;
                ...
                match field.into_value() {
                    ...
                    Opt::Async(val, span) => {
                        if async_configured {
                            return Err(Error::new(
                                span,
                                "cannot specify second async config"
                            ));
                        }
                        async_configured = true;
                        opts.async_ = val;
                    },
                    Opt::TrappableErrorType(val) => {
                        opts.trappable_error_type = val
                    },
                    ...
                    Opt::Interfaces(s) => {
                        if inline.is_some() {
                            return Err(Error::new(
                                s.span(),
                                "cannot specify a second source"
                            ));
                        }
                        inline = Some(format!(
                            "
                                package \
                             wasmtime:component-macro-synthesized;
...
fn parse_source(
    ...
    let mut parse = |resolve: &mut Resolve,
                     path: &Path|
     -> anyhow::Result<_> {
        if path.is_dir() {
    ...
    let inline_pkg = if let Some(inline) = inline {
        Some(resolve.push(UnresolvedPackage::parse(
            "macro-input".as_ref(),
            &inline
        )?)?)
    } else {
    ...
    fn parse(input: ParseStream<'_>) -> Result<Self> {
        ...
        if l.peek(kw::path) {
            ...
            if input.peek(syn::LitBool) {
                ...
                let ctor: fn(HashSet<String>) -> AsyncConfig =
                    if l.peek(kw::except_imports) {
            ...
            let fields: Punctuated<_, Token![,]> = contents
                .parse_terminated(
                    trappable_error_field_parse,
                    Token![,]
                )?;
            Ok(Opt::TrappableErrorType(Vec::from_iter(
                fields.into_iter()
            )))
            ...
            let fields: Punctuated<(String, String), Token![,]> =
                contents
                    .parse_terminated(with_field_parse, Token![,])?;
...
fn trappable_error_field_parse(
    input: ParseStream<'_>
) -> Result<TrappableError> {
    let wit_path = input.parse::<syn::LitStr>()?.value();
    input.parse::<Token![=>]>()?;
    let rust_type_name =
        input.parse::<syn::Path>()?.to_token_stream().to_string();
    Ok(TrappableError {
        wit_path,
        rust_type_name
    })
...
fn with_field_parse(
    ...
    let append =
        |buf: &mut String, segment: syn::PathSegment| -> Result<()> {
